...
title: 1-jvm-内存模型
date: 2024-07-23 16:12:35
tags:

---

![](/images/JVM_mem_module.png)
...
Java对象由JVM分配。首先，JVM使用内存分配器为对象分配出足够的空间。一种简单的内存分配器实现为碰撞指针。碰撞指针将可用内存分为两部分：已用内存和未用内存。
通过移动分隔这两部分的指针来完成内存分配。C/C++的内存分配器与这类实现有显著不同，主要是因为Java中不存在指针，JVM可以随意移动对象，来减少内部碎片 。

然后，JVM将对分配的空间进行初始化。具体来说，即填写Header中的字段，并将Instance data初始化为0。完成空间初始化后，JVM再调用类的构造函数完成用户所需的对象构造。
用户的构造函数总是可以假设它需要处理的成员已经被初始化为0。
